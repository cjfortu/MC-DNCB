...
class ConstrainedBayesOpt():
    def __init__(self, all_actions_dict, contexts, kernels, constraint_thres, constr_greater, noise=1e-6, points=[], 
        ...
        if optimizedKernels == 1:
            ...
            with warnings.catch_warnings():
                ...
                start = time.time()
                # print('\nConstrainedBayesOpt:\npoints:\n{}rewards:\n{}\n'.format(points, rewards))
                gp_hyp_objective.fit(points, rewards)
                elapsed_time = time.time() - start
            self.kernel_opt_time.append(elapsed_time)
            print(f'Done! - time: {elapsed_time}')
    ...
    def suggest(self, context):
        ...
        if len(self._space) < self.init_random:
            ...
            suggestion = self._space.random_sample_from_S_syn(context)
        else:
        ...
        return self._space.array_to_action(suggestion), self._space.empty_safeset

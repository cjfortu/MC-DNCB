...
class DistributionalCBBase:
    def __init__(self, env, noise, batch_size, actor_loss_function, critic_objective_class,
    ...
    def normalize_state(self, state):
        state_norm = (state + self.state_high) / (2 * self.state_high)
        # print('state: {}, state_norm: {}'.format(state, state_norm))
        return state_norm
    ...
    def run(self, n_iters, evaluation=0, alpha_exec=None):
        ...
        for step in range(n_iters-1):
            ...
            new_state, reward, done, info = self.env.step(denorm_action, alpha=alpha)
            ...
            if evaluation == 0:
                # print(state[0])
                self.memory.push(state[0], action, rew1, rew2)
...
class DistributionalCB(DistributionalCBBase):
    def __init__(self, env, noise, batch_size, actor_kwargs, critics_kwargs, actor_loss_function, critic_loss, critic_objective_class,
                 convnet_kwargs=None, actor_learning_rate=1e-4, critics_learning_rate=1e-3, 
                 max_memory_size=2000, save_model=0, updates_per_step=1, device='cpu'):
        super().__init__(env, noise, batch_size, actor_loss_function, critic_objective_class,
                         save_model, updates_per_step, device)
        ...
        if convnet_kwargs is None:
            actor_kwargs.update({'in_features' : self.dim_states, 'out_features' : self.dim_actions, 'device' : device})
    ...
    def update(self):
        ...
        rews1 = torch.FloatTensor(np.array(rewards)).to(self.device)
        rews2 = torch.FloatTensor(np.array(constraints)).to(self.device)
        
        rewsall = [rews1, rews2]
        
        # Critics loss
        for i, (critic, rews) in enumerate(zip(self.critics, rewsall)):
            
            Qvals = critic.forward(states, actions)
            obj = self.critic_objective.get_objective(rews, i)
        ...
        policy_loss =  self.actor_loss_function.compute_loss(self.actor, self.critics, states)

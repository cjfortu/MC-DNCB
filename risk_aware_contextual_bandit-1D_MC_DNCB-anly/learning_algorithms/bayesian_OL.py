...
class Bayesian_OL:
    ...
    def __init__(self, env, beta_val=2.5, contexts={'c1': '', 'c2': ''}, actions={'a': np.linspace(0, 2 * np.deg2rad(90), 1001)}, constr_val=144.16040391163045, constr_greater=0, 
                 points=[], rewards=[], constraint_vals=[]):
        self.env = env
        ...
        safeset = np.array([[0.4 * 2 * np.deg2rad(90)]])
    ...
    def run(self, n_iters, evaluation=0, alpha_exec=None):
        ...
        for step in range(n_iters-1):
            ...
            new_state, metrics, done, info = self.env.step([action['a']], alpha=empty_safeset)

...
class constrained_loss:
    def __init__(self, distrib_critic, quantile_critic, constr_op='max', device='cpu'):
    ...
    def compute_loss(self, actor, critics, states):
        ...
        for i in range(len(critics)):
            ...
            policy_loss += critics[i].forward(states, actor.forward(states)).mean(1).reshape(-1,1)
...
class critic_objective:
    def __init__(self, n_critics, constr_val=0, constr_op='max', device='cpu'):
    ...
    def get_objective(self, reward, idx):

...
do
    # for sigma in $sigmas; do
        tag="syn_var$sigmas-rep$r"
        
        algo="MC_DNCB"
        tag_c="$tag"
        $pythonver $script --algo $algo -T $timestamp --env_noise_var $sigmas -t $tag_c --quant_critic $qc
        ...
        # algo="RANCB"
        # tag_c="$tag-conf3"
        # $pythonver $script --algo $algo -T $timestamp --env_noise_var $sigmas -t $tag_c -a $conf3 --constr_val $constr_val --quant_critic $qc
        ...
        # algo="RANCB"
        # tag_c="$tag-conf9"
        # $pythonver $script --algo $algo -T $timestamp --env_noise_var $sigmas -t $tag_c -a $conf9 --constr_val $constr_val --quant_critic $qc
